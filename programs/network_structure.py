...
edited from https://github.com/weiaicunzai/pytorch-cifar100
...
class BottleNeck(nn.Module):
    # Residual block: resnet over 50 layers
    expansion = 4
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, args):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(args.n_channels , 64, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True))
...
def resnet9(args):
    return ResNet(BasicBlock, [1, 1, 1, 1], args)
...
def select_network(args, device):
    ...
    elif args.network == "resnet9":
        model = resnet9(args).to(device)
    ...
    if not hasattr(torch.backends, 'mps'):
        print("Wrap the model with DataParallel")
